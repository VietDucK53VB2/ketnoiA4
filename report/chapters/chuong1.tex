...
\begin{tikzpicture}[
    ...
    box/.style={rectangle,draw=black,line width=0.8pt,minimum height=1.05cm,align=center},
    flow/.style={-{Latex[length=2.2mm]},line width=0.8pt}
]
    \node[box,minimum width=2.6cm] (input) at (0,0) {Ảnh đầu vào\\640 x 640};
    \node[box,minimum width=2.9cm] (backbone) at (3.4,0) {Backbone\\trích xuất đặc trưng};
    \node[box,minimum width=2.6cm] (neck) at (6.8,0) {Neck\\gộp đa tỉ lệ};
    \node[box,minimum width=2.5cm] (head) at (10.0,0) {Head\\dự đoán};
    \node[box,minimum width=2.9cm] (output) at (13.2,0) {Bounding box\\class + confidence};

    \draw[flow] (input) -- (backbone);
    \draw[flow] (backbone) -- (neck);
    \draw[flow] (neck) -- (head);
    \draw[flow] (head) -- (output);

    \draw[black,line width=0.6pt] (-0.95,-1.55) rectangle (0.95,-0.65);
    \draw[black,line width=0.6pt] (-0.65,-1.35) rectangle (0.15,-0.82);
    \node at (0,-1.9) {Input image};

    \node[box,minimum width=1.7cm,minimum height=0.65cm] at (3.4,-1.1) {C2f};
    \node[box,minimum width=1.7cm,minimum height=0.65cm] at (6.8,-1.1) {FPN/PAN};
    \node[box,minimum width=1.7cm,minimum height=0.65cm] at (10.0,-1.1) {Detect};
    \node[align=center] at (13.2,-1.25) {person 0.91\\vehicle 0.87};
\end{tikzpicture}
\caption{Kiến trúc tổng quan của YOLOv8}
\label{fig:yolov8-architecture}
\Nguon{Nhóm thực hiện tổng hợp từ tài liệu Ultralytics}
\end{figure}
...
\begin{sodo}[H]
    ...
    \begin{tikzpicture}[
        font=\ReportDiagramFont,
        ext/.style={rectangle,draw=black,line width=0.8pt,minimum width=3.0cm,minimum height=0.9cm,align=center},
        svc/.style={rectangle,draw=black,line width=0.8pt,minimum width=3.2cm,minimum height=0.8cm,align=center},
        flow/.style={-{Latex[length=2.2mm]},line width=0.8pt}
    ]
        \node[ext] (camera) at (0,1.8) {A2\\Camera Stream};
        \node[ext] (core) at (11.5,2.7) {A6\\Core Business};
        \node[ext] (analytics) at (11.5,0.9) {A5\\Analytics};

        \draw[dashed,line width=0.7pt] (3.0,-0.25) rectangle (8.75,3.75);
        \node[anchor=south east,font=\bfseries\ReportDiagramFont] at (8.75,3.75) {AI Vision Service (A4)};

        \node[svc] (api) at (5.85,3.0) {API Router\\\texttt{/vision/detect}};
        \node[svc] (engine) at (5.85,1.85) {Detection Engine\\YOLOv8n / Mock AI};
        \node[svc] (adapter) at (5.85,0.7) {Adapter + Store\\DetectionResult};

        \draw[flow] (camera.east) -- node[midway,above,fill=white,inner sep=1pt]{\texttt{frame\_url}} (api.west);
        \draw[flow] (api) -- (engine);
        \draw[flow] (engine) -- (adapter);
        \draw[flow] (adapter.east) -- node[midway,above,sloped,fill=white,inner sep=1pt]{Core event} (core.west);
        \draw[flow] (adapter.east) -- node[midway,below,sloped,fill=white,inner sep=1pt]{Detection log} (analytics.west);
    \end{tikzpicture}
    }
...
\begin{sodo}[H]
    ...
    \begin{tikzpicture}[
        ...
        relation/.style={draw=black,line width=0.7pt},
        boundary/.style={draw=black,dashed,line width=0.7pt}
    ]
        \node[actor] (camera) at (0,4.3) {Camera Stream\\(A2)};
        \node[actor,minimum width=2.3cm,minimum height=0.65cm] (admin) at (0,1.7) {Admin / DevOps};
        \node[actor] (core) at (11.4,3.0) {Core Business\\(A6)};

        \node[usecase] (detect) at (5.2,4.25) {Phân tích ảnh\\(\texttt{POST /detect})};
        \node[usecase] (lookup) at (5.2,2.3) {Tra cứu kết quả\\(\texttt{GET /detections/:id})};
        \node[usecase] (event) at (5.2,0.65) {Tạo camera event\\(\texttt{POST /events/camera})};
        \node[usecase] (health) at (5.2,-1.35) {Kiểm tra sức khỏe\\(\texttt{GET /health})};

        \draw[boundary] (1.25,-2.05) rectangle (10.55,4.75);
        \node[anchor=south east,font=\bfseries\ReportDiagramFont] at (10.55,4.75) {AI Vision Service};

        \draw[relation] (camera.east) -- (detect.west);
        \draw[relation] (admin.east) -- (lookup.west);
        \draw[relation] (admin.east) -- (event.west);
        \draw[relation] (admin.south east) -- (health.north west);
        \draw[relation] (core.west) -- (event.east);
        \draw[relation] (core.south west) -- (health.north east);
    \end{tikzpicture}
...
\begin{sodo}[H]
    ...
    \begin{tikzpicture}[
        font=\ReportDiagramFont,
        layer/.style={rectangle,draw=black,line width=0.8pt,minimum width=7.2cm,minimum height=0.72cm,align=center},
        client/.style={rectangle,draw=black,line width=0.8pt,minimum width=2.35cm,minimum height=0.72cm,align=center},
        flow/.style={-{Latex[length=2.2mm]},line width=0.8pt}
    ]
        \node[client] (browser) at (0,3.45) {Browser};
        \node[client] (postman) at (0,2.25) {Postman};
        \node[client] (camera) at (0,1.05) {Camera\\Stream};

        \draw[line width=0.8pt] (3.0,-0.95) rectangle (11.75,4.05);
        \node[anchor=south west,font=\bfseries\ReportDiagramFont] at (3.0,4.05) {Host Machine};
        \node[layer] (docker) at (7.35,3.35) {Docker Engine};
        \node[layer] (container) at (7.35,2.45) {Container: ai-vision-service};
        \node[layer] (uvicorn) at (7.35,1.55) {Uvicorn ASGI Server};
        \node[layer] (fastapi) at (7.35,0.65) {FastAPI Application};
        \node[layer] (model) at (7.35,-0.25) {YOLOv8n Model + Mock Fallback};

        \draw[flow] (browser.east) -- node[midway,above,fill=white,inner sep=1pt]{\texttt{/docs}} (docker.west);
        \draw[flow] (postman.east) -- node[midway,above,fill=white,inner sep=1pt]{REST} (container.west);
        \draw[flow] (camera.east) -- ++(0.75,0) |- node[pos=0.72,left,fill=white,inner sep=1pt]{detect} (fastapi.west);
        \draw[flow] (fastapi) -- (model);
    \end{tikzpicture}
