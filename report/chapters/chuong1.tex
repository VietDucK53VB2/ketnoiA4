...
Docker đóng gói ứng dụng vào container, giúp mọi môi trường chạy được đồng nhất. Với bài tập lớn, Docker giúp thành viên trong nhóm và giảng viên có thể chạy service trong môi trường giống hệt nhau, chỉ bằng một lệnh `docker compose up`.
...
\begin{enumerate}
    ...
    \item Tải và giải mã ảnh bằng `httpx` và OpenCV.
...
AI Vision Service nhận request từ Camera Stream với schema `VisionDetectRequest`:
...
\hline
frame_url & string(URL) & Có & URL của ảnh cần phân tích, hỗ trợ alias `image_url`. \\
\hline
...
\hline
model_mode & string & `yolo` hoặc `mock-ai`. \\
\hline
...
A2 - Camera Stream là bên gọi chính vào AI Vision. Khi phát hiện chuyển động, Camera Stream tạo `frame_url` và gọi `POST /api/v1/vision/detect`.

\subsection{Dịch vụ downstream}

A6 - Core Business nhận kết quả phát hiện qua adapter endpoint `POST /api/v1/events/camera` để áp dụng rule nghiệp vụ. A5 - Analytics nhận detection log để tổng hợp thống kê.
...
\begin{sodo}[H]
    ...
    A2 Camera Stream (Provider)\\
    $\downarrow$\\
    A4 - AI Vision Service\\
    Input: `frame_url`\\
    Xử lý: YOLOv8n / Mock AI\\
    Output: `DetectionResult`\\
    Adapter: `CoreCameraEvent`\\
    $\downarrow$\\
...
\begin{sodo}[H]
    ...
    Camera Stream (A2): phân tích ảnh\\
    Admin/DevOps: kiểm tra sức khỏe service\\
    Core Business (A6): tạo camera event\\
