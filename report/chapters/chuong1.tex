...
\begin{tabular}{|p{2.2cm}|p{3.4cm}|p{4.1cm}|p{2.8cm}|}
\hline
...
Docker đóng gói ứng dụng vào container, giúp mọi môi trường chạy được đồng nhất. Với bài tập lớn, Docker giúp thành viên trong nhóm và giảng viên có thể chạy service trong môi trường giống hệt nhau, chỉ bằng một lệnh \texttt{\detokenize{docker compose up}}.
...
\begin{enumerate}
    ...
    \item Tải và giải mã ảnh bằng \texttt{\detokenize{httpx}} và OpenCV.
...
AI Vision Service nhận request từ Camera Stream với schema \texttt{\detokenize{VisionDetectRequest}}:
...
\hline
camera_id & string & Có & Mã định danh camera. \\
\hline
frame_url & string(URL) & Có & URL của ảnh cần phân tích, hỗ trợ alias \texttt{\detokenize{image_url}}. \\
\hline
...
\hline
detection_id & string & Mã duy nhất của detection. \\
\hline
camera_id & string & Mã camera từ request. \\
\hline
frame_url & string & URL ảnh đã phân tích. \\
\hline
timestamp & datetime & Thời điểm phân tích (UTC). \\
\hline
anomaly_detected & boolean & Có phát hiện bất thường hay không. \\
\hline
confidence_score & float/null & Độ tin cậy, chỉ có khi anomaly_detected = true. \\
\hline
finding & object/null & Chi tiết đối tượng phát hiện. \\
\hline
model_mode & string & \texttt{\detokenize{yolo}} hoặc \texttt{\detokenize{mock-ai}}. \\
\hline
...
A2 - Camera Stream là bên gọi chính vào AI Vision. Khi phát hiện chuyển động, Camera Stream tạo \texttt{\detokenize{frame_url}} và gọi \texttt{\detokenize{POST /api/v1/vision/detect}}.

\subsection{Dịch vụ downstream}

A6 - Core Business nhận kết quả phát hiện qua adapter endpoint \texttt{\detokenize{POST /api/v1/events/camera}} để áp dụng rule nghiệp vụ. A5 - Analytics nhận detection log để tổng hợp thống kê.
...
\begin{sodo}[H]
    ...
    Input: \texttt{\detokenize{frame_url}}\\
    Xử lý: YOLOv8n / Mock AI\\
    Output: \texttt{\detokenize{DetectionResult}}\\
    Adapter: \texttt{\detokenize{CoreCameraEvent}}\\[2pt]
    $\downarrow$\\[2pt]
