...
\begin{sodo}[H]
    ...
    \begin{tikzpicture}[
        ...
        \node[anchor=south east,font=\bfseries\ReportTableFont] at (8.75,3.75) {AI Vision Service (A4)};
        ...
        \draw[flow] (camera.east) -- node[above]{\texttt{frame\_url}} (api.west);
        \draw[flow] (api) -- (engine);
        \draw[flow] (engine) -- (adapter);
        \draw[flow] (adapter.east) -- node[above,sloped]{CoreCameraEvent} (core.west);
        \draw[flow] (adapter.east) -- node[below,sloped]{Detection log} (analytics.west);
    \end{tikzpicture}
...
\begin{sodo}[H]
    ...
    \begin{tikzpicture}[
        ...
        \node[anchor=south east,font=\bfseries\ReportTableFont] at (10.55,4.75) {AI Vision Service};
...
\begin{sodo}[H]
    ...
    \begin{tikzpicture}[
        font=\ReportTableFont,
        layer/.style={rectangle,draw=black,line width=0.8pt,minimum width=7.6cm,minimum height=0.85cm,align=center},
        client/.style={rectangle,draw=black,line width=0.8pt,minimum width=2.8cm,minimum height=0.8cm,align=center},
        flow/.style={-{Latex[length=2.2mm]},line width=0.8pt}
    ]
        \node[client] (browser) at (0,3.6) {Browser};
        \node[client] (postman) at (0,2.35) {Postman};
        \node[client] (camera) at (0,1.1) {Camera Stream};

        \draw[line width=0.8pt] (3.1,-0.35) rectangle (12.3,4.25);
        \node[anchor=south west,font=\bfseries\ReportTableFont] at (3.1,4.25) {Host Machine};
        \node[layer] (docker) at (7.7,3.45) {Docker Engine};
        \node[layer] (container) at (7.7,2.55) {Container: ai-vision-service};
        \node[layer] (uvicorn) at (7.7,1.65) {Uvicorn ASGI Server -- port 8000};
        \node[layer] (fastapi) at (7.7,0.75) {FastAPI Application};
        \node[layer] (model) at (7.7,-0.15) {YOLOv8n Model + Mock Fallback};

        \draw[flow] (browser.east) -- node[above]{\texttt{/docs}} (docker.west);
        \draw[flow] (postman.east) -- node[above]{REST API} (container.west);
        \draw[flow] (camera.east) -- node[below]{\texttt{/api/v1/vision/detect}} (fastapi.west);
