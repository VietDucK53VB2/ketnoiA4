...
\begin{enumerate}
    ...
    \item Camera Stream chạy motion detection. Khi phát hiện có chuyển động, nó tạo `frame_url` và gọi `POST /api/v1/vision/detect`.
    \item AI Vision tải ảnh, phân tích và trả về `DetectionResult`.
...
\begin{itemize}
    \item Endpoint: `POST /api/v1/vision/detect`
    \item Method: POST
    \item Content-Type: application/json
    \item Request: `VisionDetectRequest`
    \item Response 200: `DetectionResult`
    \item Response 422: `ProblemDetails` khi `frame_url` không hợp lệ hoặc không tải được
\end{itemize}

\subsection{Phân tích độ tin cậy}

\begin{itemize}
    \item Timeout: `httpx` client có timeout 15 giây.
    \item Retry: hiện chưa có cơ chế retry ở tầng API.
    \item Idempotency: mỗi request tạo một `detection_id` mới, request giống hệt nhau gửi 2 lần vẫn tạo 2 detection khác nhau.
\end{itemize}
...
Nhóm implement Adapter Pattern để chuyển `DetectionResult` sang `CoreCameraEvent`.
...
AI Vision không chỉ trả về kết quả phân tích mà còn tự động đề xuất quyết định `core_decision`. Điều này giúp Core Business không cần implement lại logic phân tích.
...
\begin{sodo}[H]
    \centering
    \fbox{\parbox[c][5.2cm][c]{0.82\textwidth}{\centering
    Camera IP $\rightarrow$ A2 Camera Stream $\rightarrow$ A4 AI Vision $\rightarrow$ A6 Core Business $\rightarrow$ A7 Notification\\
...
\hline
OpenAPI Contract & 20\% & Tốt & `openapi.yaml`, schema, example, error model, Swagger UI \\
\hline
...
\hline
Trình bày & 15\% & Tốt & `PRESENTATION.md`, kịch bản 1 phút, thứ tự demo rõ ràng \\
\hline
\end{tabular}
\Nguon{Nhóm thực hiện}
\end{table}
...
\hline
\textbf{Trịnh Việt Đức} & Nhóm trưởng / Backend Lead & Điều phối nhóm, thiết kế kiến trúc, phát triển `main.py`, `vision.py`, xử lý ảnh, YOLO integration, làm việc với A2/A6/A5, tổng hợp nội dung báo cáo & 50\% \\
\hline
\textbf{Nguyễn Quang Đạt} & API/Contract Owner & Viết `openapi.yaml`, Postman collection, smoke test, tài liệu API, hỗ trợ kiểm thử hợp đồng và minh chứng chạy thử & 25\% \\
\hline
\textbf{Lê Quang Dũng} & DevOps / Report & Docker, docker-compose, minh chứng vận hành, hỗ trợ tổng hợp báo cáo, phụ lục và kiểm tra bản dựng & 25\% \\
\hline
\end{tabular}
\Nguon{Nhóm thực hiện}
\end{table}
...
\hline
\centering\textbf{Nguyễn Quang Đạt} & \centering\textbf{Trịnh Việt Đức} & \centering\textbf{Lê Quang Dũng} \\
\hline
...
\begin{itemize}
    \item `DETECTION_STORE` là Python dict trong RAM, mất toàn bộ dữ liệu khi restart.
